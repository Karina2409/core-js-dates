...
function getNextFriday(date) {
  if (date.getDay() < 4)
    return new Date(date.setDate(date.getDate() + 5 - date.getDay()));
  if (date.getDay() > 4)
    return new Date(date.setDate(date.getDate() + 7 - (date.getDay() - 5)));
  return date;
}
...
function isDateInPeriod(date, period) {
  ...
  return currentDate <= endPeriod && currentDate >= startPeriod;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= 31; day += 1) {
    ...
    if (date.getDay() === 0 || date.getDay() === 6) weekendCount += 1;
  }
...
function getNextFridayThe13th(date) {
  const nextDate = new Date(date.getFullYear(), date.getMonth(), 13);
